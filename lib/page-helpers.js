...
(() => {
  ...
  async function scrapeVideos(maxScrollMs) {
    ...
    // 视频播放页（/watch?v= 或 /shorts/ID）：只返回当前视频，忽略推荐/侧栏
    const wm = location.href.match(/[?&]v=([\w-]{11})/) || location.pathname.match(/\/shorts\/([\w-]{11})/);
    if (wm && (location.pathname === '/watch' || location.pathname.indexOf('/shorts/') === 0)) {
      const id = wm[1];
      const tEl = document.querySelector('ytd-watch-metadata h1, #title h1, h1.ytd-watch-metadata, h1.title');
      let title = ((tEl && tEl.textContent) || document.title)
        .replace(/^\(\d+\)\s*/, '')
  ...
  async function scrapeBili(maxPages) {
    ...
    // 视频播放页（/video/BV…）：只返回当前这一个视频，忽略右侧推荐视频
    const wm = location.pathname.match(/\/video\/(BV[\w]+)/);
    if (wm) {
      const id = wm[1];
      const tEl = document.querySelector('h1.video-title, h1[title], .video-title-text, .video-title');
      let title = clean((tEl && (tEl.getAttribute('title') || tEl.textContent)) || document.title.replace(/[_\-]哔哩哔哩.*$/, ''));
      return [{ id, title: title || id }];
    }
